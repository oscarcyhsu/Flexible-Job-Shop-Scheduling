...
def job_shop(slices, jobs, machine_avalible_time, time_out):
	...
	start_time_bool = []
	for time in range(horizon - min_start):
		temp_start_time = []
		for job_id, job in enumerate(jobs):
			for op_id, op in enumerate(job[1]):
				temp_m = []
				for m_id in range(slices):
					temp_m.append(model.NewBoolVar('start_time_bool' + (' job %d op %d' % (job_id, op_id))))
				temp_start_time.append(temp_m)
		start_time_bool.append(temp_start_time)
	...
	for job_id, job in enumerate(jobs):
		...
		for op_id, op in enumerate(job[1]):
			...
			for m_id in range(slices):
				...
				for time in range(horizon - min_start):
					# (time == l_start - min_start)  # target

					# temp_time = model.NewIntVar(0, horizon, 'time')
					# model.Add(temp_time == (l_start - min_start))
					cond = model.NewBoolVar('cond')
					model.Add(time == (l_start - min_start)).OnlyEnforceIf(cond)
					model.Add(time != (l_start - min_start)).OnlyEnforceIf(cond.Not())
					model.Add(start_time_bool[time][temp_id][m_id] == 1).OnlyEnforceIf(cond)
					model.Add(start_time_bool[time][temp_id][m_id] == 0).OnlyEnforceIf(cond.Not())
	...
	for time in range(horizon - min_start):
		prev = sum([2 ** (len(start_time_bool[time]) - op) * start_time_bool[time][op][0] for op in range(len(start_time_bool[time]))])
		for m in range(1, slices):
			now = sum([2 ** (len(start_time_bool[time]) - op) * start_time_bool[time][op][m] for op in range(len(start_time_bool[time]))])
			model.Add(prev >= now)
			prev = now
...
def _args():
	...
	parser.add_argument('group_size', type=int, default=8, help="decomposition group size")
	parser.add_argument('time_out', type=float, default=960.0, help="solver timeout")
