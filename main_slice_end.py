...
def decomposed_job_shop(slices, jobs, output_file_path, group_size, time_out):
	...
	n_selected_job = 0
	total_weight = sum([job[0] for job in jobs])
	remain_weight = total_weight
	while n_selected_job < len(ordered_jobs):
		...
		remain_weight -= sum([job[0] for job in selected_jobs])
		print(f"selected jobs {selected_jobs}")
		# schedule these jobs
		solver, status, ops_start, presences = job_shop(slices, selected_jobs, machine_avalible_time, time_out, remain_weight)  # solve problem
...
def job_shop(slices, jobs, machine_avalible_time, time_out, remain_weight):
	...
	for job_id, job in enumerate(jobs):
		...
		for op_id, op in enumerate(job[1]):
			...
			for m_id in range(slices):
				...
				l_presence_end = model.NewIntVar(0, horizon, 'end' + m_suffix)

				l_presences.append(l_presence)
				l_indexs.append(l_index)
				slice_ends[m_id].append(l_presence_end)
				...
				model.Add(l_presence_end == l_end).OnlyEnforceIf(l_presence)
				model.Add(l_presence_end == 0).OnlyEnforceIf(l_presence.Not())
	...
	# find slice max end
	slice_max_ends = []
	for m_id, ends in enumerate(slice_ends):
		max_end = model.NewIntVar(min_start, horizon, 'min end % d' % m_id)
		model.AddMaxEquality(max_end, ends)
		slice_max_ends.append(max_end)
	
	max_ends_sum = cp_model.LinearExpr.ScalProd(slice_max_ends, [int(1) for _ in range(slices)])
	...
	model.Minimize(cp_model.LinearExpr.Sum([cp_model.LinearExpr.Term(makespan, 10000), weighted_sum, cp_model.LinearExpr.Term(max_ends_sum, int(10000/slices)*int(remain_weight))]))
