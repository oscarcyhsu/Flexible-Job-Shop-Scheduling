...
def main():
    args = _args()
    slices, jobs = read_input(args.input_file_path)

    if slices == 1:
        single_machine(jobs, args.output_file_path)
    else:
        op_count = sum([len(job[1]) for job in jobs])
        if op_count <= 20:
            decomposed_job_shop(slices, jobs, args.output_file_path, len(jobs), 240.0, True)
        else:
            if slices < 10 and op_count < 90:
                decomposed_job_shop(slices, jobs, args.output_file_path, 8, 240.0, True)
            else:
                decomposed_job_shop(slices, jobs, args.output_file_path, args.group_size, args.time_out)


def decomposed_job_shop(slices, jobs, output_file_path, group_size, time_out, small_case=False):
    ...
    while n_selected_job < len(ordered_jobs):
        ...
        # schedule these jobs
        solver, status, ops_start, presences = job_shop(slices, selected_jobs, machine_avalible_time, time_out, small_case)  # solve problem

        if solver.StatusName(status) == 'UNKNOWN':
            GROUP_SIZE = 1
            selected_jobs = ordered_jobs[n_selected_job : min(n_selected_job + 1, len(ordered_jobs))]
            solver, status, ops_start, presences = job_shop(slices, selected_jobs, machine_avalible_time, 960.0, small_case)
...
def job_shop(slices, jobs, machine_avalible_time, time_out, small_case=False):
    ...
    weight = [int(job[0] * 100000) for job in jobs]  # w_i
    ...
    model.Minimize(cp_model.LinearExpr.Sum([cp_model.LinearExpr.Term(makespan, 100000), weighted_sum]))
...
def single_machine(jobs, output_file_path):

    # Sort job_id by weight-duration-ratio in descending order (Greedy Choice)
    def weight_duration_ratio(job):
        weight = int(1000000 * job[0])
        job_duration = sum(op[1] for op in job[1])
        return weight / job_duration

    job_order = sorted(range(len(jobs)), key=lambda x: weight_duration_ratio(jobs[x]), reverse=True)

    # Record start time for each job
    job_start_value = collections.defaultdict(int)
    current_value = 0
    for job_id in job_order:
        job_start_value[job_id] = current_value

        job_duration = sum(op[1] for op in jobs[job_id][1])
        current_value += job_duration

    # Deal with operations order
    with open(output_file_path, 'w') as f:
        for job_id, job in enumerate(jobs):

            # Topological sort
            dependency = {op_id + 1: set(op[2]) for op_id, op in enumerate(job[1])}
            topo_order = toposort_flatten(dependency)

            # Record start time for each operation
            current_value = job_start_value[job_id]
            op_start_value = collections.defaultdict(int)
            for op_id in topo_order:
                op_start_value[op_id - 1] = current_value

                op_duration = job[1][op_id - 1][1]
                current_value += op_duration

            # Output
            for op_id in range(len(job[1])):
                print(op_start_value[op_id], 1)
                f.write(f'{op_start_value[op_id]} 1 \n')
...
def _args():
    ...
    parser.add_argument('input_file_path', type=Path, help='input data path')
    parser.add_argument('output_file_path', type=Path, help='output data path')
    parser.add_argument('-g', '--group_size', type=int, default=7, help="decomposition group size")
